 ...
 */
package com.slim.respository.http;

import android.support.annotation.Nullable;

import com.orhanobut.logger.Logger;
import com.slim.utils.ZipHelper;
...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (logRequest) {
            ...
            //打印请求信息
            Logger.w("Params : 「 %s 」%nConnection : 「 %s 」%nHeaders : %n「 %s 」"
        ...
        //            //打印响应时间以及响应头
        //            Logger.w("Received response in [ %d-ms ] , [ %s ]%n%s"
    ...
    private String printResult(Request request, Response response, boolean logResponse) throws IOException {
        ...
        if (isParseable(responseBody.contentType())) {
            ...
            if (logResponse) {
                Logger.d("响应结果:--->" + request.url());
                //               Logger.w(String.format(  "%s \n %s",request.url(),isJson(responseBody.contentType()) ?
                //                       CharacterHandler.jsonFormat(bodyString) : isXml(responseBody.contentType()) ?
                //                       CharacterHandler.xmlFormat(bodyString) : bodyString));
            }
        } else {
            if (logResponse) {
                Logger.w("This result isn't parsed");
            }
        }
        return bodyString;
    }
